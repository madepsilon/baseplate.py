...
def http_adapter_from_config(
    ...
    """Make an HTTPAdapter from a configuration directory.

    The keys useful to :py:func:`http_adapter_from_config` should be
    prefixed, e.g. ``http.pool_connections``, ``redis.max_retries``, etc. The
    ``prefix`` argument specifies the prefix used. Each key is mapped to a
    corresponding keyword argument on the
    :py:class:`~requests.adapters.HTTPAdapter` constructor.
    ...
    * ``pool_block``: Whether the connection pool will block when trying to get
    ...
    parser = config.SpecParser(
        {
            ...
            "filter": {
                "ip_whitelist": config.Optional(config.TupleOf(ipaddress.ip_network)),
                "ip_blacklist": config.Optional(config.TupleOf(ipaddress.ip_network)),
                "port_whitelist": config.Optional(config.TupleOf(int)),
                "port_blacklist": config.Optional(config.TupleOf(int)),
                "hostname_blacklist": config.Optional(config.TupleOf(config.String)),
    ...
    #     "validator",
    #     AddrValidator(
    #         ip_whitelist=options.filter.ip_whitelist,
    #         ip_blacklist=options.filter.ip_blacklist,
    #         port_whitelist=options.filter.port_whitelist,
    #         port_blacklist=options.filter.port_blacklist,
    #         hostname_blacklist=options.filter.hostname_blacklist,
...
class InternalRequestsClient(config.Parser):
    ...
    only be used to speak to trusted services on the same network.
    ...
    def parse(self, key_path: str, raw_config: config.RawConfig) -> RequestsContextFactory:
        ...
        #
        # the whitelist takes precedence. allow loopback and 10./8 private
        ...
        adapter = http_adapter_from_config(raw_config, prefix=f"{key_path}.", **self.kwargs)
